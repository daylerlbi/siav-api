...
public interface MatriculaRepository extends JpaRepository<Matricula, Long> {
    ...
    @Query("SELECT m FROM Matricula m" +
            ...
            " AND m.estadoMatriculaId.id IN (1, 2, 4)")
        // 1=Aprobado, 2=En curso 4=Reprobado
    ...
    @Query("SELECT CASE WHEN COUNT(m) > 0 THEN true ELSE false END " +
            ...
            "WHERE m.estudianteId = :estudiante " +
            "AND m.grupoCohorteId.grupoId.materiaId = :materia " +
            "AND m.estadoMatriculaId.id IN (1, 2)")
    // 1=Aprobado, 2=En curso
    ...
    @Query("SELECT DISTINCT m.estudianteId FROM Matricula m WHERE m.semestre = :semestre AND m.grupoCohorteId = :grupoCohorte AND m.estadoMatriculaId.id IN (1, 2, 4)")
        // 1=Aprobado,
        // 2=En
        // curso,
        // 4=Reprobado
