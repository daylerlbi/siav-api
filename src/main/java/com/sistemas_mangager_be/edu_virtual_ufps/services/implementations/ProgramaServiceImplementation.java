...
public class ProgramaServiceImplementation implements IProgramaService {
        ...
        if (programaRepository.existsByCodigo(programaDTO.getCodigo())) {
            throw new ProgramaExistsException(
                    String.format(IS_ALREADY_USE,
                            "El código de programa " + programaDTO.getCodigo()));
        }
    ...
    public void vincularMoodleId(MoodleRequest moodleRequest)
        ...
        if (programaRepository.existsByMoodleId(moodleRequest.getMoodleId())) {
            throw new ProgramaExistsException(
                    String.format(IS_ALREADY_USE,
                            "El moodleId de programa " + moodleRequest.getMoodleId()));
        }
    ...
    public SemestreProgramaResponse listarSemestresPorPrograma(Integer programaId)
        ...
        Programa programa = programaRepository.findById(programaId)
                .orElseThrow(() -> new ProgramaNotFoundException(
                        String.format(IS_NOT_FOUND, "EL PROGRAMA CON EL ID " + programaId)
                                .toLowerCase()));
    ...
    private SemestreProgramaResponse mapToSemestreProgramaResponse(Programa programa) {
        ...
        List<SemestreProgramaResponse.SemestreResponse> semestreResponses = semestresPrograma.stream()
                .map(semestrePrograma -> {
                    ...
                    return SemestreProgramaResponse.SemestreResponse.builder()
    ...
    @Override
    public ProgramaDTO listarProgramaPorCodigo(String codigo) throws ProgramaNotFoundException {
        Optional<Programa> programaOpt = programaRepository.findByCodigo(codigo);

        if (!programaOpt.isPresent()) {
            throw new ProgramaNotFoundException("Programa con código " + codigo + " no encontrado");
        }
        ...
        return ProgramaDTO.builder()
                ...
                .moodleId(programa.getMoodleId())
                .historicoMoodleId(programa.getHistoricoMoodleId())
                .semestreActual(programa.getSemestreActual())
                .build();
    }
}
