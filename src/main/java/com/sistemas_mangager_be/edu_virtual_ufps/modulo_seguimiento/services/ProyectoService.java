...
public class ProyectoService {
    ...
    public ProyectoDto actualizarProyecto(Integer id, ProyectoDto proyectoDto) {
        ...
        if (existente.getEstadoRevision() == EstadoRevision.EN_REVISION) {
            throw new RuntimeException("El proyecto no puede ser editado mientras está en revisión.");
        }

        existente.setUpdatedAt(LocalDate.now());

        //solo actualiza el estado si el rol es docente, admin o superadmin
        if (existente.getEstadoRevision() == EstadoRevision.ACEPTADA) {
            if (!"Docente".equalsIgnoreCase(rol) && !isAdmin) {
                throw new RuntimeException("Solo el docente o administrador puede actualizar el estado.");
            }

            Integer nuevoEstadoCode = proyectoDto.getEstadoActual();
            if (nuevoEstadoCode == null) {
                throw new RuntimeException("Debe enviar el nuevo estado.");
            }
            ...
            if (nuevoEstado.ordinal() == estadoActual.ordinal() + 1 || nuevoEstado.ordinal() == 0) {
                existente.setEstadoActual(nuevoEstado);
                existente.setEstadoRevision(EstadoRevision.SIN_REVISAR);
                existente.setComentarioRevision(null);
                existente.setUpdatedAt(LocalDate.now());
                return proyectoMapper.toDto(proyectoRepository.save(existente));
            }
            throw new RuntimeException("El proyecto no puede ser editado.");
        } else {
            proyectoDto.setEstadoActual(existente.getEstadoActual().ordinal());
        }
        //solo actualiza el estado si el rol es docente, admin o superadmin
...
//        }
    ...
    @Transactional
    @PreAuthorize("hasAuthority('ROLE_ESTUDIANTE') or hasAuthority('ROLE_DOCENTE') or hasAuthority('ROLE_SUPERADMIN') or hasAuthority('ROLE_ADMIN')")
    public void validarFase(int idProyecto, EstadoRevision estadoRevision, String comentarioRevision) {
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();

        Proyecto existente = proyectoRepository.findById(idProyecto)
                .orElseThrow(() -> new RuntimeException("Proyecto no encontrado"));
        ...
        if (isAdmin) {
            existente.setEstadoRevision(estadoRevision);
            existente.setComentarioRevision(comentarioRevision);
            ...
            if (activo.getRolId().getNombre().equals("Estudiante")) {
                if (
                        existente.getEstadoRevision() == EstadoRevision.RECHAZADA ||
                        existente.getEstadoRevision() == EstadoRevision.SIN_REVISAR ||
                        existente.getEstadoRevision() == null
                ) {
                    existente.setEstadoRevision(EstadoRevision.EN_REVISION);
                    existente.setComentarioRevision(null);
                    proyectoRepository.save(existente);
                } else if (
                        existente.getEstadoRevision() == EstadoRevision.EN_REVISION
                ) {
                    existente.setEstadoRevision(EstadoRevision.SIN_REVISAR);
                    existente.setComentarioRevision(null);
                    proyectoRepository.save(existente);
                }
            } else if (activo.getRolId().getNombre().equals("Docente")) {
                existente.setEstadoRevision(estadoRevision);
                existente.setComentarioRevision(comentarioRevision);
                proyectoRepository.save(existente);
            }
    ...
    public DefinitivaDto calcularYAsignarDefinitiva(Integer idProyecto, TipoSustentacion tipoSustentacion) {
        ...
        if (proyecto.getEstadoActual() != EstadoProyecto.FASE_8) {
            throw new RuntimeException("Solo se puede asignar la calificación en la fase de " + EstadoProyecto.FASE_8.getDescripcion() + " (" + EstadoProyecto.FASE_8.name() + ")");
        }
